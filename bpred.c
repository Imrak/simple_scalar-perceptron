...
((ADDR >> MD_BR_SHIFT) % ((PRED)->config.perceptron_list.size))
		...
		case BPredPerc:
			...
			* If the branch is conditional. Get the direction pointer from the decision function of the perceptron.
			...
			if((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND)){
				...
				//int loc = ipoly_41(baddr); // using i-poly hashing function
				int loc = ipoly_37_reduced(baddr);
				//printf("In prediction hash: address: %d, index: %d\n", baddr, loc);
...
struct bpred_update_t *dir_update_ptr	/* pred state pointer */
	...
	* NOTE if(!!x == !!y) is equivalent to if( ((x > 0) && (y > 0)) || ((x == 0) && (y == 0)))
	...
	if((MD_OP_FLAGS(op) & (F_CTRL|F_COND)) == (F_CTRL|F_COND)){
		if((pred->class == BPredPerc) || (pred->class == BPredPerc_D))
		{
			//int loc = PERCEP_HASH(pred->dirpred.perceptron, baddr); // using mod hashing function
			//int loc = ipoly_41(baddr); // using i-poly hashing function
			int loc = ipoly_37_reduced(baddr);
			//printf("In training hash: address: %d, index: %d\n", baddr, loc);
